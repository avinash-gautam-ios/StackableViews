...
class ASampleViewController: UIViewController, UIStackable {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    view.backgroundColor = UIColor.colorFrom(red: 21, green: 25, blue: 34)
    addBottomCTA()
  }

  private func addBottomCTA() {
    bottomButton.setTitle("Show Options", for: .normal)
  ...
  private func addHeaderCTA() {
    headerButton.setTitle("Header View #1", for: .normal)
    headerButton.backgroundColor = UIColor.colorFrom(red: 21, green: 25, blue: 34)
  ...
  @objc private func bottomCTAPressedAction() {
    let sampleB = BSampleViewController()
    try? stackController?.stack?.push(sampleB)
  }
  ...
  func stackableController(_ controller: UIStackableController, didUpdateState state: UIStackableState, forItem item: UIStackable) {
    print("state changed for \(NSStringFromClass(ASampleViewController.self)) to \(state)")
    switch state {
    case .collapsed:
      addHeaderCTA()
    default: break
